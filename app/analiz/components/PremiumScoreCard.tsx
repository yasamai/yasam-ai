...
export default function PremiumScoreCard({
  ...
  return (
    <article style={{
      ...
      background: `linear-gradient(145deg, ${palette.bg} 0%, #ffffff 100%)`,
      boxShadow: "0 12px 30px rgba(15,23,42,.07)",
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: "16px" }}>
        <div>
          <p style={{ margin: 0, color: "#475569", fontSize: "13px", fontWeight: 800 }}>{title}</p>
          <div style={{ display: "flex", alignItems: "flex-end", gap: "5px", marginTop: "10px" }}>
            <strong style={{ color: palette.text, fontSize: "38px", lineHeight: 1 }}>{safeValue}</strong>
            <span style={{ color: "#94a3b8", fontSize: "13px", paddingBottom: "4px" }}>{suffix}</span>
          </div>
        </div>
        <span style={{
          width: "12px", height: "12px", borderRadius: "999px",
          background: palette.accent, boxShadow: `0 0 0 7px ${palette.border}`, marginTop: "5px"
        }} />
      </div>

      <div style={{ width: "100%", height: "8px", marginTop: "16px", borderRadius: "999px", background: "#e2e8f0", overflow: "hidden" }}>
        <div style={{ width: `${safeValue}%`, height: "100%", borderRadius: "999px", background: palette.accent }} />
      </div>
