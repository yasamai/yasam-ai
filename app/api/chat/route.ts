/*
YAŞAM AI - V44.1 GERÇEK OPENAI ROUTE
BU DOSYA SADECE ŞURAYA KONULACAK:
app/api/chat/route.ts

Bu sürüm:
- Kullanıcı girişi olmasa da gerçek OpenAI raporu üretir.
- Giriş yapan kullanıcı varsa Supabase'e kaydetmeyi dener.
- Giriş yoksa raporu yine ekranda gösterir.
- Demo/sabit sonuç döndürmez.
*/
  ...
  try {
    ...
    if (!apiKey) {
      return errorResponse(
        "OpenAI API anahtarı bulunamadı. Sunucu ayarlarını kontrol edin.",
        500,
      );
    }
    ...
    if (!contentType.includes("application/json")) {
      return errorResponse("İstek JSON biçiminde gönderilmelidir.", 415);
    }
    ...
    let databaseResult: { saved: boolean; id: string | null } = {
      saved: false,
      id: null,
    };

    if (authenticatedUserId) {
      databaseResult = await saveReport(
        userMessage,
        report,
        authenticatedUserId,
      );
    ...
    console.error(`[${requestId}] Yaşam AI API hatası:`, error);
    ...
    if (status === 400) {
      return errorResponse(
        "Analiz isteği işlenemedi. Girilen bilgileri kontrol edip tekrar deneyin.",
        400,
      );
    }
    ...
    if (status === 408) {
      return errorResponse(
        "Analiz isteği zaman aşımına uğradı. Tekrar deneyin.",
        504,
      );
    }
